...
\usepackage{graphicx,pgffor,afterpage}
\usepackage[left=10px,right=10px,top=10px,bottom=10px,paperwidth=8in,paperheight=25in]{geometry}
\usepackage{float}
\usepackage{tikz}
...
\tikzset{chatstyle/.style={text width=4.5in,font=\sffamily,rounded corners=2pt,minimum height=0.8cm}}
...
\def\chatline#1\par{%
   ...
   \ifodd\thechatlinenum
       \hspace{0.8\textwidth}
       \tikz[]{\node[fill=mygreen,chatstyle,align=left]{\strut#1\unskip\strut}; \node[chatstyle] (img) at (-1,0) {\includegraphics[height=0.7cm]{images/user1.png}};}%
   \else
       \hfill
       \tikz[]{ \hspace{0.1\textwidth} \node[fill=lightgray,chatstyle]{\strut#1\strut};
       \node[chatstyle] (img) at (-1.1,0) {\includegraphics[height=0.7cm]{images/user2.png}}; }%
   \fi
   \par
   \smallskip
}
...
\foreach \name in \names {%
    \begin{subfigure}[]{}
        \includegraphics[width=0.3\linewidth]{images/\name}
    \end{subfigure}\quad
}
\end{figure}
...
%		\foreach \name in \namesc {%
%  		  \begin{subfigure}[]{}
% 		       \includegraphics[width=0.2\linewidth]{images/\name}
% 	 	  \end{subfigure}\quad
%		}
%	\end{figure}
%
